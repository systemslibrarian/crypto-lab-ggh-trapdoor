 ...
 *
 * IN THIS LAB BOTH HALVES ARE LIVE, which is unusual in this fleet.
 *
 * The control-boundary half judges a small, sharp-edged set: the dimension
 * range slider (`#dim-slider`), the error-mode `<select>` (`#sigma-select`),
 * seven buttons — keygen, encrypt, the two decrypt buttons, the Break 1 stepper
 * and the Break 2 run control — and the shared top bar's `.cl-btn` links.
 *
 * The slider is the interesting one. 1.4.11 exempts a component "whose
 * appearance is determined by the user agent and not modified by the author",
 * and the guard further down honours that by skipping `appearance: auto` inputs
 * of type range/checkbox/radio/color/file — so a stock slider is not judged.
 * The moment it takes `appearance: none` and an author-drawn track and thumb —
 * the usual next step for a lab that wants its slider in the accent colour —
 * the exemption is gone and every pixel of it is answerable here. That is a
 * cascade fact, not a source fact, which is why it is measured at every driven
 * state rather than read out of the stylesheet once.
 *
 * The `<select>` is the opposite case and it is why the generated-content half
 * is live rather than inert: §4.2 of the template REQUIRES a styled select to
 * be `appearance: none` plus a custom chevron, so it is author-painted from the
 * start, and that chevron is a `::before`/`::after` glyph. A pseudo-element is
 * not an element and owns no text node, so neither axe nor the text walk in
 * `contrast.ts` can reach its ink. This file is the only thing that measures it.
 *
 * Two more shapes on this page are worth naming because they are where 1.4.11
 * failures actually come from: a control whose border is repainted the SAME
 * colour as its own fill has no edge of its own and lives or dies by
 * fill-vs-surround, and a control whose only delineator is a low-percentage
 * `color-mix()` toward a decorative divider token reads as a 1.3–2.5:1 edge
 * however sound the token looked when it was declared. Both are measured as
 * painted, at every driven state — including the disabled and hover repaints
 * only the drive reaches.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
    ...
    // `over`, `fade`, `lum` and `ratio` come from the paint core below, which is
    // the same model `contrast.ts` uses — the two files must agree about what a
    // pixel is, or a control's edge and the text sitting on it would be judged
     ...
     * declares a gradient is not asserted here — that is exactly the kind of
     * fact that is true until someone adds a wash behind the parallelepiped
     * cloud. The sampler is the fleet's shared paint model and stays whole, so
     * the first gradient is measured on arrival rather than silently misread.
     */
     ...
     * page, not a tablist, so nothing is ever off-screen in the DOM sense — and
     * the expensive part is the matrices. A basis pair at dimension 60 is two
     * 60×60 integer grids, and the same walk also covers the U·V = I proof, the
     * ciphertext strip with every entry of e called out, and the Break 1
     * stepper's residual vectors: several thousand cells, all siblings
    ...
    // Declared by the shared paint core above, and referenced here so
    // `noUnusedLocals` does not reject the parts of the model this file's two
    // checks reach only through `paintAt`. Do not delete one because it "looks
    // unused" — deleting the symbol deletes the model with it.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
     ...
     * measuring the theme. §4.2 of the template requires this page to paint its
     * canvas with the `background-color` LONGHAND on `<html>` (the shorthand is
     * what axe and WebKit miss), which is the declaration that makes the root
     * resolve on the first try and leaves the `<body>` fallback dormant. Both
     * halves stay because either declaration is one refactor away from
     * vanishing, and this lab's canvas is near-black: fall through to WHITE here
     * and every control on the page reports a fabricated edge ratio. The
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // 129 of the 131 copies of this file in the fleet still have that bug;
      // this one does not, and that is the reason this lab's gate was copied
      // from the one repo that carries the fix rather than from a recent
      // neighbour. A one-sided edge is not exotic here either: an underline
      // under the active act's heading link, or a single accent rule marking
      // the row of the basis a readout is describing, is the ordinary way this
      // page would delineate something.
      //
      ...
      // 1px a coloured state will later occupy so nothing shifts. Counting that
      // spacer as a border would make every control that uses the idiom report
      // 1.00:1: no fill, no painted edge, identified by its text alone, which is
      // exactly the case the "is it trying to draw itself as a control?" test
      // below exists to exclude.
